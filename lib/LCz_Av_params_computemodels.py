...
class LCz_Av_params(object):
                    ...
                    for idx, ww in zip(idxbest, weights):
                        ...
                        if self.doref:
                           ...
                           # LCintref = np.interp(np.median(self.mjdref[mask]) - texp, self.times * stretch, self.allmags[band][idx][iz + int(idxz)][iAv + int(idxAv)])
                            LCintref = np.interp(self.mjdref[mask][0] - texp, self.times * stretch, self.allmags[band][idx][iz + int(idxz)][iAv + int(idxAv)])
    ...
    def doMCMC(self, **kwargs):
        ...
        #commented lines of h5
        # filename containing sample chain (using .h5)
       # import h5py
       # import os.path
       # from os import path

       # if path.exists("samples/chain_%s_%s_%s.h5" % (self.modelname, self.objname, self.fitlabels)):
        #    os.remove("samples/chain_%s_%s_%s.h5" % (self.modelname, self.objname, self.fitlabels))
         #   print("File Removed!")
          #  chainname_h5 = "samples/chain_%s_%s_%s.h5" % (self.modelname, self.objname, self.fitlabels)
           # print("File Restored!")
       # else:
        #    chainname_h5 = "samples/chain_%s_%s_%s.h5" % (self.modelname, self.objname, self.fitlabels)
         #   print("File Created!")
       ...
       # chainname = "samples/chain_%s_%s_%s.dat" % (self.modelname, self.objname, self.fitlabels)
        ...
        # backend
       # backend = emcee.backends.HDFBackend(chainname_h5)
  ...
  # load samples
        if doload:
            ...
            # self.labels nombre de los parametros
            f = open(chainname, "w")
            f.write("niter nwalker %s\n" % (" ".join(self.labels)))
 
#mas cosas comentadas de h5           
            # prueba buscar cadenas sampler.sample (barra de progreso para generar get_chain)
           # print("terreno PANTANOSO")
           # for sample in sampler.sample(pos, iterations = self.nsteps, progress=True):
            #    continue
           # print("funciono sampler.sample!!!!")
           # get_chain = sampler.get_chain()
            # print("guardamos la get_chain()")
            
           # for idx, result in enumerate(get_chain):
               # for k in range(result.shape[0]):
                  # if np.mod(idx, 2) == 0:
              #          print("%s %s %4i %4i %s" % (self.modelname, self.objname, idx, k, " ".join(map(lambda p: str(p), result[k]))))
               #     f.write("%i %i %s\n" % (idx, k, " ".join(list(map(lambda p: str(p), result[k])))))
           # f.close()
           # print("se cerro el archivo chainname")
                

            # self.chain debe ser el sampler de la cadena
           # self.chain = get_chain
        
           #old version 

            for idx, result in enumerate(sampler.sample(pos, iterations = self.nsteps)):
                ##result.log_prob get te array of log_prob from results (results contains array of log_prob, blobs and ramdom state?)#
               # print(sampler.sample(pos, iterations = self.nsteps))
               # print(type(sampler.sample(pos, iterations = self.nsteps)))
               # print(enumerate(sampler.sample(pos, iterations = self.nsteps)))
...
#commented lines are from alex's
        fig, ax = plt.subplots(nrows = self.ndim, sharex = True, figsize = (16, 10))
        for j in range(self.ndim):
            ax[j].set_ylabel(self.labels[j])
            for i in range(self.nwalkers):
               ...
               # for n in range(len(self.chain)):
                #    ite_chain.append(self.chain[n, :, j][i])
               # ax[j].plot(ite_chain, alpha = 0.2)
        plt.savefig("plots/MCMC_%s_%s_%s_evol.png" % (self.modelname, self.objname, self.fitlabels))
       ...
       # print("produciendo nuevo samples corner")

        # extract samples from the archive h5

       # filename = "samples/chain_%s_%s_%s.h5" % (self.modelname, self.objname, self.fitlabels)
       # reader_h5 = emcee.backends.HDFBackend(filename)

        # corner samples
       
       # samples = reader_h5.get_chain(discard = nburn).reshape((-1, self.ndim))


        # do corner plot
        print("Doing corner plot...")
        # aca iba la variable samplescorner
        # fig = corner.corner(samples, labels = self.labels)#, truths = self.bestfit)
        fig = corner.corner(samplescorner, labels = self.labels)#, truths = self.bestfit)
        #print("se logro el corner.corner")
