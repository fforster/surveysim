...
class LCz_Av_params(object):
    ...
    def compute_models(self, **kwargs):
        ...
        for filename, params in zip(self.files, self.params):
            ...
            for band in bands:
                ...
                npydir = "%s/npy/%s/%s/%s" % (os.environ["SURVEYSIM_PATH_INPUT"], self.modelname, obsname, band)
                if doload:
    ...
    def doMCMC(self, **kwargs):
        ...
        sampler = emcee.EnsembleSampler(self.nwalkers, self.ndim, self.lnprob, backend=backend)
        print("acabo SAMPLER")
