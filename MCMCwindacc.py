...
if __name__ == "__main__":
    ...
    # set metric
    LCs.setmetric(metric = np.array([1., 1., 1., 1., 10., 1.]), logscale = np.array([False, False, True, False, False, True], dtype = bool))
    ...
    if dointeractive:
        ...
        log10mdot_slider = Slider(log10mdot_slider_ax, 'log mdot', LCs.parbounds[LCs.parlabels == 'log10mdot'][0][0], LCs.parbounds[LCs.parlabels == 'log10mdot'][0][1], valinit=LCs.parvals[LCs.parlabels == 'log10mdot'][0], dragging = False)
        ...
        def slider_update(val):
            ...
            for band in LCs.uniquefilters:
                ...
                ax.relim()
                ax.autoscale_view()
                ax.set_title("scale: %5.3f, texp: %f, Av: %f, mass: %f, energy: %f, mdot: %3.1e, rcsm: %3.1f, beta: %f" % (scale, texp, np.exp(logAv), mass, energy, 10**log10mdot, rcsm, beta), fontsize = 8)
    ...
    priors = np.array([lambda scale: norm.pdf(scale, loc = 1., scale = 0.01), \
                       ...
                       lambda log10mdot: uniform.pdf(log10mdot, loc = -7, scale = 5), \
